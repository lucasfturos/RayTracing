#include "Scenes/BuleUtah/bule.hpp"
...
int main(int argc, char *argv[]) {
    try {
        ...
        auto bule{make_shared<BuleUtah>()};
        ...
        case 3:
            switch (opc[1]) {
            case 1:
                render = make_shared<Render>(bule->single_scene(), 1);
                render->run();
                break;
            case 2:
                render = make_shared<Render>(bule->single_scene(), 0);
                render->run_ppm();
                break;
            default:
                break;
